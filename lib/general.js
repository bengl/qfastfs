...
const hasTypes = (major === 10 && minor >= 10) || major > 10;
const hasRecursive = (major === 10 && minor >= 12) || major > 10;

// This whole chunk is just needed for isdir
const { S_IFDIR, S_IFMT } = fs.constants;
const statValues = bStatValues || getStatValues();
const hasBigInt = (major >= 10 && minor >= 5) || major > 10;
const statFunc = hasBigInt ? (file, req) => bStat(file, false, req) : bStat;

const FSWrap = FSReqWrap || FSReqCallback;
class Wrap extends FSWrap {
  constructor (cb) {
    super();
    this.oncomplete = cb;
  }
